...
int pipewrite(struct pipe* pi, uint64 addr, int n) {
    ...
    while (w < n) {
        ...
        if (pi->nwrite == pi->nread + PIPESIZE) {  // DOC: pipewrite-full
            ...
            size = MIN(MIN(n - w, pi->nread + PIPESIZE - pi->nwrite),
                       PIPESIZE - (pi->nwrite % PIPESIZE));
            if (copyin(p->pagetable, &pi->data[pi->nwrite % PIPESIZE], addr + w,
                       size) < 0) {
                panic("copyin");
            }
            pi->nwrite += size;
...
int piperead(struct pipe* pi, uint64 addr, int n) {
    ...
    while (r < n && size != 0) {  // DOC: piperead-copy
        ...
        if (copyout(p->pagetable, addr + r, &pi->data[pi->nread % PIPESIZE],
                    size) < 0) {
            panic("copyout");
        }
        pi->nread += size;
